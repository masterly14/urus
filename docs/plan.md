...
13. [Implementación consolidada — Microsite, NLU comprador y suite de evaluación IA](#implementación-consolidada--microsite-nlu-comprador-y-suite-de-evaluación-ia)
...
- **Vía API REST** (clientes, propiedades, propietarios): Cliente HTTP autenticado con token estático (`Token: ...`). CRUD directo, respuestas JSON tipadas, sin necesidad de Playwright ni sesiones. Rate limits: 10 propiedades/min, 20 clientes/min, 20 propietarios/min.
- **Vía RPA Legacy** (demandas, cambios de estado, geo-polígonos): Login silente con Playwright → captura cookies de sesión → raspa token CSRF → dispara XHR clonado a `guardar.php`. Necesario porque la API REST no cubre demandas (que requieren polígonos geoespaciales) ni cambios de estado operativo.
- **Ingestion Worker** (lectura): Cron-job en Node.js. Usa API REST (`GET /propiedades/?listado`) para detectar cambios por `fechaact` y `GET /propiedades/?cod_ofer` para datos completos. Usa polling legacy para demandas (no cubiertas por REST). Emite eventos inmutables hacia la Capa 3.
- **Egestion Worker** (escritura): Para clientes/propiedades/propietarios usa API REST (`POST/PUT/DELETE`). Para demandas y estados usa RPA legacy.
...
Inmovilla dispone de una API REST v1 (`https://procesos.inmovilla.com/api/v1`) que cubre CRUD de clientes, propiedades y propietarios. Autenticación por token estático (sin login, sin 2FA, sin cookies). El principio de segregación de responsabilidades se mantiene, pero la implementación se simplifica donde la API tiene cobertura:

- **Se mantiene:** Capa 3 (Event Store, Job Queue, reglas de negocio e IA), observabilidad, SLAs, idempotencia y proyecciones.
- **Se simplifica:** Lectura/escritura de clientes, propiedades y propietarios usa API REST directa en vez de RPA.
- **Se mantiene legacy:** Demandas (requieren polígonos geoespaciales, no cubiertos por REST), cambios de estado operativo, y cualquier operación no expuesta por la API.
...
1. Puerto interno de integración (`InmovillaReadPort` / `InmovillaWritePort`) para desacoplar la lógica de negocio.
2. Adaptador REST para clientes/propiedades/propietarios y adaptador legacy para demandas/estados.
3. Feature flag (`INMOVILLA_MODE=legacy|rest|hybrid`) con fallback automático.
4. Estrategia de lectura: listado REST (`fechaact`) para detección de cambios + GET individual para los que han cambiado. Fallback a polling legacy para carga masiva si rate limits son insuficientes.

---

### API REST de Statefox — Proveedor de datos de mercado (solo lectura)

Statefox expone una API REST (`https://statefox.com/public/aapi/props`) de solo lectura con Bearer token. Se consume exclusivamente por API, sin necesidad de RPA, Playwright ni ingeniería inversa.
...
#### Lo que Statefox NO ofrece

- Sin endpoints de escritura (no se sincronizan demandas)
- Sin generación de enlaces privados / microsites
- Sin webhooks / push notifications
- Sin gestión de búsquedas automatizadas

#### Consecuencia arquitectónica

Statefox se usa **100% como proveedor de datos vía API**. Los microsites de selección para compradores se generan localmente con Next.js (microsite propio con branding Urus Capital). Esto elimina toda necesidad de RPA para Statefox.
...
| Firma digital          | **Motor in-house** (Next.js)  | PDF, hash, `/firma/{token}`, OTP SMS, eventos Neon — ver `docs/firma-digital.md`                                                                                                                               |
...
```
M0 (Infra)
├── M1 (Ingestion: API REST + legacy) ──┬── M2 (Egestion: API REST + legacy) ──┬── M5 (Smart Matching)
│                                        │                                       ├── M7 (Pricing) ← M6
│                                        │                                       └── M8 (Smart Closing)
│                                        ├── M3 (Scoring) ──────────────────────┘
│                                        └── M10 (Dash Comercial) ──┐
...
| Búsqueda de stock externo en mercado        | M6 (Statefox API REST)      |
| Microsite de selección para compradores     | M6 (Microsite Next.js)      |
| Validación del comercial                    | Capa 4 micro-frontend + M4  |
| Envío al comprador y feedback loop          | M4, M5, M6 (microsite)      |
| Motor de Pricing                            | M7                          |
| Smart Closing                               | M8                          |
...
| SMS (OTP firma)        | Credenciales en `.env` (`VONAGE_*`) si se usa el canal actual para códigos                                                                                               | ☐ Pendiente |
...
DEVELOP_DATABASE_URL=postgresql://...
PROD_DATABASE_URL=postgresql://...

# OpenAI (LangGraph + Whisper)
OPENAI_API_KEY=...

# Inmovilla — API REST (clientes, propiedades, propietarios)
INMOVILLA_API_TOKEN=...  # Generado en Ajustes > Opciones > Token para API Rest

# Inmovilla — RPA legacy (demandas, estados)
INMOVILLA_USER=...
INMOVILLA_PASSWORD=...
INMOVILLA_OFFICE_KEY=...

# Composio (2FA de Inmovilla vía Gmail)
COMPOSIO_API_KEY=...
COMPOSIO_USER_ID=...

# Upstash QStash (scheduler de cron-jobs)
QSTASH_URL=...
QSTASH_TOKEN=...
...
# Firma digital in-house (Sprint 2) — ver .env.example: FIRMA_TOKEN_SECRET, VONAGE_*, SIGNATURIT_* (SLA/conversor PDF, nombres históricos)

# Google Calendar (a configurar en Sprint 1, Semana 2)
...
- Playwright instalable (`npx playwright install`) — solo necesario para flujo legacy de demandas
...
1. Mover issues/tasks al estado correcto.
2. Si hay PR lista: auto-review rápido antes de dejarla.
...
> **Objetivo:** Levantar la infraestructura, integrar las APIs REST de Inmovilla y Statefox, implementar RPA legacy para demandas, y tener el pipeline de leads operativo con scoring, WhatsApp y Smart Matching.

### Semana 1 — Infraestructura Base + Integración APIs REST + Ingeniería Inversa Legacy
...
- `CONTACTO_INGESTADO`, `SLA_INICIADO`
- `DEMANDA_ACTUALIZADA`, `MATCH_GENERADO`

**Nota terminológica — "Lead" en este plan:**
En Inmovilla **no existe una entidad "Lead"**. Lo que el sector llama "lead" se mapea a dos entidades reales: un **Contacto** (persona con nombre, teléfono, email — consultable por API REST `GET /clientes/`) y una **Demanda** (búsqueda activa con criterios y polígono geoespacial — solo accesible por RPA legacy). Cuando un comprador llega desde un portal (Idealista, Fotocasa), Inmovilla crea automáticamente un Contacto + una Demanda vinculada. La API REST ofrece lectura de estos mensajes de portales vía `GET /propiedades/?leads`, pero son mensajes sobre propiedades, no una entidad independiente. En el resto de este plan, "lead" se usa como concepto de negocio (un potencial comprador/vendedor que entra al sistema) que se materializa como **Contacto + Demanda** en Inmovilla.
...
**Nota de alcance:** La API REST de Inmovilla cubre CRUD de clientes, propiedades y propietarios con autenticación por token estático (sin login, sin 2FA, sin cookies). Las demandas, estados operativos y datos geoespaciales (polígonos de búsqueda) **no están cubiertos** por la API REST y requieren ingeniería inversa legacy (`guardar.php`, sesión con Playwright). El login 2FA legacy requiere **Composio + Gmail** para extraer el código de verificación automáticamente. El flujo de colaboradores externos se gestiona desde el dashboard interno por el Comercial/CEO, con Neon como estado operativo (Inmovilla no modela hitos operativos de bancos/abogados/tasadores, por lo que no hay datos de colaboradores que reconciliar desde Inmovilla). Los colaboradores externos no acceden al sistema.
...
- ✅ Ingestion Worker leyendo propiedades (API REST) y demandas (legacy) de Inmovilla.
- ✅ Egestion Worker escribiendo clientes/propiedades (API REST) y demandas (RPA legacy).
...
| AM     | 11:00–13:00 | Implementar **backend del microsite**: persistir en Neon qué propiedades se mostraron, a qué comprador, cuándo. Fichas con imágenes (`pImages`), precio, metros, zona, extras. **En producción el feedback del comprador es solo por WhatsApp** (NLU contextual → `SELECCION_COMPRADOR`); los botones web del plan original quedaron sustituidos por este canal único para evitar sesgo y unificar trazabilidad. | Backend + tracking funcional   |
| PM     | 13:00–15:00 | Implementar **orquestación IA del microsite**: aprobación automática + mejora de descripciones + evento `SELECCION_VALIDADA` + envío del enlace al comprador por WhatsApp sin intervención comercial.                                                                                                                                                                                                         | Flujo IA end-to-end |
...
| AM     | 10:00–12:00 | **DEMO:** Flujo completo en vivo: lead entra → scoring → WhatsApp al comercial → respuesta del comprador → Smart Matching → cruce → consulta Statefox API → microsite generado → aprobación IA → envío enlace al comprador |
...
- ✅ Statefox API REST integrada + Microsite de selección para compradores.
- ✅ Micro-frontends de post-visita y agenda.
- ✅ Workers resilientes con retry y logging (API REST + RPA legacy).
...
| Bloque | Horario     | Tarea                                                                                                                                                                                                                                                                                                                                                                                                    | Entregable                     |
| ------ | ----------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------ |
| AM     | 8:30–11:00  | **Firma digital in-house:** envío a firma (`POST /api/contracts/sign`), página pública `/firma/{token}`, verificación **OTP por SMS**, generación de PDF sellado + audit trail, persistencia y eventos en Neon. **Recordatorios y escalados vía WhatsApp Cloud API** según SLAs y cadencia de esta tabla. Documentar plantillas Meta y variables de entorno en `docs/firma-digital.md`.                  | Firma + spec SLA               |
...
- **SLA de firma completa:** **5 días naturales** desde el envío efectivo de la solicitud hasta que la firma quede **completada** en Neon (fuente de verdad: estado y evento `FIRMA_COMPLETADA`). Plazo **parametrizable** (config/env) para afinar por tipo de documento u operación.
- **Cadencia de recordatorios WhatsApp** (mientras el estado sea pendiente en Neon): envíos en **día +1**, **día +3** y **día +5** (días naturales desde el envío). El mensaje del día +5 indica que es el **último recordatorio automático** antes del escalado por SLA.
...
- **Separación de responsabilidades:** la **confirmación** de firma y los artefactos (PDF sellado, audit trail) se generan en la propia aplicación al completar `/api/firma/{token}/sign`; los recordatorios WhatsApp son **seguimiento operativo** y no sustituyen ese cierre.
...
- **Documentación obligatoria para implementar:** en **README** (sección Smart Closing / Firma) y [docs/firma-digital.md](firma-digital.md): plantillas WhatsApp, variables (`FIRMA_TOKEN_SECRET`, `VONAGE_`*, prefijos `SIGNATURIT_*` donde apliquen SLA/conversor), y eventos Neon (`FIRMA_*`).
...
| -------------------------------- | ------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `contrato_firma_recordatorio_d1` | Recordatorio firmante **día +1**                                                                  | **{{1}}** nombre corto · **{{2}}** tipo documento (p. ej. «Contrato de arras», coherente con `ContractDocumentKind`) · **{{3}}** referencia operación (`operationId` o stem `OP-…_Arras_vN`) · **{{4}}** URL de firma (`/firma/{token}`) |
...
| `contrato_firma_sla_escalado`    | Escalado a **comercial** y **gestor (BO)** tras SLA sin firma completa                            | **{{1}}** referencia operación · **{{2}}** tipo documento · **{{3}}** enlace seguimiento `{NEXT_PUBLIC_APP_URL}/platform/legal/contratos/{id}`                                                                                                    |
...
| AM     | 11:00–13:00 | Implementar **panel interno de gestión de colaboradores**: interfaz dentro del dashboard donde el Comercial o el CEO asigna colaboradores a operaciones, sube documentos en su nombre, y actualiza estados tipo kanban de los hitos. Los colaboradores externos **no acceden al sistema**; toda la gestión la realiza el equipo interno. Este flujo vive 100% en Neon porque Inmovilla no modela hitos operativos de colaboradores (no hay entidad banco/abogado/tasador en su modelo). No hay datos de colaboradores que reconciliar desde Inmovilla. | Panel interno funcional |
| PM     | 13:00–15:30 | Implementar **tracking de hitos**: cada cambio de estado (registrado por el Comercial/CEO) registra timestamp en Neon. Cálculo automático de tiempos. Hitos estándar por tipo (banco: documentación → estudio → preaprobación → aprobación; abogado: revisión → observaciones → validación).                                                                                                                                                                                                                                                           | Tracking funcional      |
...
| AM     | 11:00–13:00 | Implementar **feedback loop completo del comprador** (canal WhatsApp): mensaje del comprador → `WHATSAPP_RECIBIDO` → **NLU contextual LangGraph** (`classifyBuyerFeedback`: propiedades del microsite + historial) → eventos `SELECCION_COMPRADOR` por propiedad y, si aplica, `DEMANDA_ACTUALIZADA` → proyección + **Egestion** a Inmovilla (RPA legacy) → nueva consulta Statefox → `GENERATE_MICROSITE` (y `wantsMoreOptions` fuerza regeneración). Memoria en `WhatsAppBuyerSession`. Detalle en la sección **Implementación consolidada** (mismo documento) y en `docs/microsite-feedback-loop.md`. | Feedback loop end-to-end |
...
| AM     | 10:00–12:00 | **DEMO MES 1:** Recorrido completo del sistema: dashboards CEO y comercial, Smart Closing con voz, Motor de Pricing, Gestión de colaboradores, cadencias post-venta |
...
## Implementación consolidada — Microsite, NLU comprador y suite de evaluación IA

> **Abril 2026.** Aterrizaje en código de decisiones de producto y QA que amplían lo descrito en las tablas semanales de Sprint 1–2. Documentación detallada: `docs/microsite-feedback-loop.md`, `docs/nlu-eval-suite.md`.

### Microsite como portal de marca (M6)

- **Experiencia cerrada:** el comprador permanece en el dominio Urus Capital; no se enlaza al portal externo de Statefox.
- **Listado:** `/seleccion/{token}` con tarjetas navegables a **ficha detalle** `/seleccion/{token}/propiedad/{propertyId}`.
- **Curación enriquecida:** `MicrositeCuratedProperty` incluye descripción, €/m², metros (útiles/parcela/terraza), planta, orientación, coordenadas (mapa estático), certificado energético, estado/año, anunciante, carrusel ampliado de imágenes.
- **Control IA total:** la selección se aprueba y envía automáticamente; la validación manual `/validar-seleccion/*` queda como contexto histórico retirado del runtime.

### NLU contextual (LangGraph) para feedback por WhatsApp

- **Entrada:** texto del comprador + resumen de propiedades mostradas en el microsite activo + historial conversacional (turnos recientes).
- **Salida estructurada:** intención global (`ME_ENCAJA` / `NO_ME_ENCAJA` / `BUSCO_DIFERENTE`), `propertyFeedback[]` por `propertyId`, extracción de variables de demanda, bandera `wantsMoreOptions`.
- **Orquestación:** grafo en `lib/agents/nlu-graph.ts` (resolución de contexto → clasificación); el worker `whatsapp-nlu-handler` emite eventos y encola regeneración cuando corresponde.
- **Sesión:** `WhatsAppBuyerSession` (Neon) asocia `waId`, `demandId`, selección activa y contadores; se rellena al enviar el microsite (`WHATSAPP_ENVIADO`).

### Suite de evaluación AI-to-AI (calidad del NLU)

- **Objetivo:** medir el NLU sin sesgo del desarrollador ni pruebas manuales exhaustivas: agente **comprador sintético** (mensajes realistas por persona), **sistema bajo test** (`classifyBuyerFeedback`), **juez** (puntuación híbrida: reglas + LLM).
- **Persistencia:** modelos `EvalRun` / `EvalResult` en Neon; KPIs agregados por categoría y persona.
- **Operación:** CLI `npm run eval:nlu`; UI interna `/eval` (lista de runs, detalle, gráficos, tabla de fallos).
- **Mitigación de riesgo:** complementa la fila de riesgos «LangGraph produce outputs inconsistentes» (validación zod + observabilidad cuantitativa en el tiempo).
...
| 3   | Firma digital in-house, OTP y recordatorios WhatsApp | Test: envío a firma → OTP y cierre en `/api/firma/.../sign`; pendiente simulado → recordatorios en +1/+3/+5 días y escalado a comercial y gestor tras SLA |
...
| LangGraph produce outputs inconsistentes          | Media | Medio   | Validación de schema con zod en outputs, fallback a reglas; **suite AI-to-AI** (`EvalRun`/`EvalResult`, CLI `npm run eval:nlu`, dashboard `/eval`) para regresión del NLU comprador | Degradar a motor de reglas puro, alertar para revisión manual                                                                         |
...
| **Microsite de selección**             | Micro-frontend Next.js propio que muestra propiedades de Statefox al comprador con branding Urus Capital. Incluye **portal interno** (listado + ficha detalle sin salir del dominio). El **feedback** del comprador se captura por **WhatsApp** (no por botones en web). Reemplaza los enlaces privados de Statefox. |
| **NLU contextual (comprador)**         | LangGraph que interpreta mensajes de WhatsApp con contexto de propiedades mostradas e historial (`classifyBuyerFeedback`). Alimenta `SELECCION_COMPRADOR`, `DEMANDA_ACTUALIZADA` y regeneración de microsite. Ver `docs/microsite-feedback-loop.md`.                                                                 |
| **Suite de evaluación NLU (AI-to-AI)** | Batería automatizada: agente comprador sintético + juez + persistencia en Neon (`EvalRun`/`EvalResult`). CLI `npm run eval:nlu`, UI `/eval`. Ver `docs/nlu-eval-suite.md`.                                                                                                                                           |
| **LangGraph**                          | Framework para orquestar flujos agénticos con LLMs. Se usa para Smart Matching, NLU de feedback de comprador, Pricing, Smart Closing y Bot Mental.                                                                                                                                                                   |
