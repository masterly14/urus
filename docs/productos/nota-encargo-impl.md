...
>
> **Nota (junio 2026 — reprogramación de horario):** Las notas en estado `PENDING` o `PENDIENTE_PROPIEDAD` se pueden reprogramar in-place desde `/platform/captacion` (`POST /api/captacion/nota-encargo/[id]/reschedule`). Se actualiza `visitDateTime`, se incrementa `scheduleGeneration` y se republican los callbacks QStash (formulario + matching-check si aplica). Los callbacks antiguos quedan invalidados por generación (`noop_stale_schedule`) y, cuando existen, se borran best-effort vía `formularioQstashMessageId` / `matchingCheckQstashMessageId`. Se emite `NOTA_ENCARGO_REPROGRAMADA`.
>
