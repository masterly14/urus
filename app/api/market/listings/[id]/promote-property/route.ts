 ...
 *
 * Promueve (en caliente) un prospecto de Inmovilla a propiedad activa.
 * No usa job queue; responde sincrónicamente.
 */
...
const bodySchema = z.object({
  ...
  tituloes: z.string().trim().max(180).optional(),
  descripciones: z.string().trim().max(20000).optional(),
  fotos: z
    .record(
      z.string(),
      z.object({
...
const postHandler = async (
  ...
  try {
    const result = await promoteProspectoToProperty({
      listingId: id,
...
export const POST = withObservedRoute(
  { method: "POST", route: "/api/market/listings/[id]/promote-property" },
  postHandler,
);
