...
- **Auditoría cruces/microsites (read-only)**: `npx tsx scripts/audit-cruces-microsites.ts --days=7` o `--demand=DEM-XXXX` — reconstruye demanda nueva → `MATCH_GENERADO` → coverage → microsite/envío y distingue paginación visible de 30 vs total real en BD. Ver `docs/auditoria-cruces-microsites.md`.
- **Diagnóstico de entrega de microsite (read-only)**: `npm run microsite:debug-delivery -- --demandId=DEM-XXXX` o `-- --waId=346...` — reconstruye conversación → jobs → selección → envío → evento de resultado para explicar por qué llegó o no llegó el microsite.
- **Auditoría geográfica de cruces (read-only por defecto)**: `npm run audit:geographic-matches -- --demand=DEM-XXXX --days=30` — recalcula compatibilidad de zona/ciudad para `MATCH_GENERADO`; con `--apply` emite `MATCH_INVALIDADO` sin borrar historial. Ver `docs/matching-geografico.md`.
- **Simulación live de matching geográfico (casi producción)**: `npm run test:matching-geographic:live -- --demand=DEM-XXXX --days=30` — ejecuta el flujo real de rematch contra Neon y reporta qué cruces se emitirían o se rechazan por geografía/deduplicación (`--apply` para emitir `MATCH_GENERADO`). Ver `docs/matching-geografico.md`.
