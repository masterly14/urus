...
import type {
  FotocasaDiscoveryEndpoint,
  FotocasaDiscoveryReport,
  FotocasaListing,
  FotocasaSeed,
} from "./types";
...
function shouldTrackEndpoint(url: string): boolean {
  try {
    ...
    return parsed.hostname.endsWith("fotocasa.es");
  } catch {
    return false;
  }
}
...
export async function runDiscoveryForSeed(params: {
...
}): Promise<FotocasaDiscoveryReport> {
  ...
  return {
    ...
    allowedByRobots: evaluateRobots(params.policy, params.seed.url).allowed,
...
export function attachDiscoveryRecorder(page: Page): {
  ...
  const endpoints = new Map<string, FotocasaDiscoveryEndpoint>();
